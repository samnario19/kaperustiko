    ...
    function printThermalReceipt($orderData) {
        try {
            ...
            $printer->text(sprintf("%-23s %8.2f\n", "Service Charge (10%):", ($orderData['totalOrderedItemsPrice'] * 1.12 * 0.10)));
            
            $printer->setEmphasis(true);
            $printer->text(sprintf("%-23s %8.2f\n", "TOTAL:", 
                $orderData['totalOrderedItemsPrice'] * (1 - $orderData['voucherDiscount'] / 100) + 
                ($orderData['totalOrderedItemsPrice'] * 1.12 * 0.10)));
            $printer->setEmphasis(false);
            ...
            $printer->text(sprintf("%-23s %8.2f\n", "VATable Sales:", $orderData['totalOrderedItemsPrice'] * 1.12));
